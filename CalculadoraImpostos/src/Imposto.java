public class Imposto {
    ...
    public void calculaICMS(float vlrproduto, float vlripi, float vlrfrete, float vlrdespesa, String ufOrigem){
        ...
        for(int i = 0; i < uf.getUFs().length; i++){
            for(int j = 0; j < uf.getUFs()[i].length; j++){
               if(uf.getUFs()[i][j] == ufOrigem){
                ...
                float resultado = getBaseICMS() * (icms / 100);
                this.vlrICMS = resultado;
                }
            }
        }
    }
